...
var QOS = {
    NAME_PANEL_COMMON: "panel",
    ID_PANEL_COMMON: "#panel",
...
function Job(write_chart, write_option)
{
...
QoS.prototype.qos_job_init = function(job_id)
{
    if (job_id in this.qos_jobs) {
        console.error("multiple jobs with the same ID", job_id);
        return
    } else {
    }

    var panel_name = QOS.NAME_PANEL_COMMON + job_id
    var string = '<div id="' + panel_name + '" class="panel"></div>';
    ...
    job = new Job(chart, option);
    this.qos_jobs[job_id] = job;
}

QoS.prototype.qos_jobs_init = function()
{
    for(var i = 0; i < this.qos_lime.l_config.jobs.length; i++) {
        job_id = this.qos_lime.l_config.jobs[i].id
        this.qos_job_init(job_id);
    }
}
...
QoS.prototype.qos_console_init = function()
{
    ...
    websocket.onmessage = function(evt) {
        ...
        if (! job_id in that.qos_jobs) {
            console.error("unexpected datapoint for job", job_id);
            return
        }
        job = that.qos_jobs[job_id]
